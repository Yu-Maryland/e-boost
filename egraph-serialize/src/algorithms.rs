...
impl EGraph {
    ...
    pub fn inline_leaves(&mut self) -> usize {
        ...
        let mut leave_to_cost = std::collections::HashMap::new();
        for (eclass, nodes) in &eclass_to_nodes {
            if nodes.len() == 1 && nodes[0].1.children.is_empty() {
                leaves.push((eclass.clone(), nodes[0].0.clone()));
                leave_to_op.insert(eclass.clone(), nodes[0].1.op.clone());
                leave_to_cost.insert(eclass.clone(), nodes[0].1.cost);
            }
        }
        ...
        for (leaf_eclass, node_id) in &leaves {
            let parents = node_to_parents.get(leaf_eclass);
        ...
        for (parent, leaf_children) in &parents_to_children {
            let additional_cost = leaf_children
                .iter()
                .map(|child_class| *leave_to_cost.get(child_class).unwrap())
        ...
        for (eclass, node_id) in &leaves {
            ...
            if node_to_parents.get(eclass).is_none() {
                continue;
            }
            n_inlined += 1;
